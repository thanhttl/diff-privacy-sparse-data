...
import wavelets.sparse_1d_wavelet_long;
...
// for running real dataset where input size exceeds MAX_INT
public class RunRealData {	
	static int sensitivity = 1;
	static double epsilon = 0.1;
	static Mechanism mechanism = Mechanism.Wavelet2; //Filter_Priority; //Filter2; //Wavelet2;  //Filter_Priority;
	...
	static int theta = 1000; //500; // 70; //40;
	...
	static boolean useMedianRelativeError = false;
...
//	static boolean useZeroIndices = false; // zero indices are stored as a list {[start, end]}
	...
	static boolean useRealData = true;
	static boolean webTrace = false;
	static boolean oneDimensional = true; //false;
	static String fileName = "data/census_income/out/grid3.txt";
	...
	static int MAX_NUM_QUERIES = (int) 1E7; // maximum number of queries evaluated for a range percentage
	static long startIndex = (long) 2.9E9; //5E7; // to start computing range queries error from this index 

	public static void main(String[] args) {			
		int noiseSeed = 35791; int sampleSeed=73937; int binomialSeed=92381;
		...
		ArrayList<NonZeroEntry_Long> inputNonZeros = new ArrayList<NonZeroEntry_Long>();
		
		ArrayList<NonZeroEntry_Long> outputNonZeros = new ArrayList<NonZeroEntry_Long>();
		...
		double[] rangePercents = {0.00001}; 
		
		if (useRealData) {
			int numDigits = (int) Math.log10(inputSize) - 1;
			int numAdded = (int) (inputSize/(Math.pow(10, numDigits))) -1;
			//rangePercents = new double[2*numDigits+1+numAdded];
			rangePercents = new double[2*numDigits+1];
...
//			System.out.println(i);
			if ( !useDyadic || mechanism == Mechanism.Wavelet1 || mechanism == Mechanism.Wavelet2) {
				...
				if (mechanism == Mechanism.Geometric) {
					if (computeError) {
						PositionComparator_Long comparator = new PositionComparator_Long();
						Collections. sort(inputNonZeros, comparator);	
						
					//	addGeometricNoise(data, output, alpha, noiseSeed + nextSeedRand.nextInt());
					//  for large input that can't fit in memory, compute partial output
						int largestRange = (int) Math.round(rangePercents[rangePercents.length-1]*inputSize);
						data = new double[largestRange + MAX_NUM_QUERIES - 1];
						output = new double[largestRange + MAX_NUM_QUERIES - 1];
						addGeometricNoise(inputNonZeros, startIndex, data, output, alpha, noiseSeed + nextSeedRand.nextInt());
					}	
					else
					...
					if (oneDimensional) {
						sparse_1d_wavelet_long wavelet;
						if (logm <= 29) {
							if (data == null)
								data = new double[(int) inputSize];						
							wavelet = new sparse_1d_wavelet_long(logm, data);
						}
						else {
							if (coeffMap == null) 
							...
							wavelet = new sparse_1d_wavelet_long(logm, coeffMap,  false);
						}
						for (NonZeroEntry_Long entry: inputNonZeros) {
...
//						}
						if (!computeError) {
							...
							// need to add laplacian to the right entries
							int largestRange = (int) Math.round(rangePercents[rangePercents.length-1]*inputSize);
							output = new double[largestRange + MAX_NUM_QUERIES - 1];
							wavelet.inverse(output, startIndex);
							
							
							data = new double[largestRange + MAX_NUM_QUERIES - 1];
							copyToArray(inputNonZeros, data, startIndex);
						}
					}
					else { // 2d wavelets
						...
						for (NonZeroEntry_Long entry : inputNonZeros) {
							long pos = entry.getPosition();
							int x = (int) pos / MULTIPLE;
							int y = (int) pos % MULTIPLE;
							wavelet.update(x, y, entry.getValue());
						}
				//		System.out.println("Num nonzero coeffs: " + wavelet.getSize());
					...
					sampleSize = anonymize(inputNonZeros, inputSize, outputNonZeros, mechanism, theta, targetSampleSize, epsilon, noiseSeed + nextSeedRand.nextInt(), sampleSeed + nextSeedRand.nextInt(), binomialSeed + nextSeedRand.nextInt());						
				}
...
//						}
					...
					//	if (mechanism == Mechanism.Geometric || mechanism == Mechanism.Wavelet2 || testingMode) {
						...
						if (mechanism == Mechanism.Geometric || testingMode) {
							...
							computeRangeQueryError(inputNonZeros, outputNonZeros, startIndex, rangeSize, rangeXZProfiles[k]);
//							outputNonZeros = new ArrayList<NonZeroEntry_Long>();
//							outputNonZeros.add(ObjectPool.allocateNonZeroEntry_Long());
//							outputNonZeros.add(ObjectPool.allocateNonZeroEntry_Long());
//							computeRangeQueryError(inputNonZeros, new ArrayList<NonZeroEntry_Long>(), startIndex, rangeSize, rangeXZProfiles[k]);
						}
...
//						System.out.println(k + ", " + absoluteErrors[k]);
				...
				ObjectPool.returnNonZeroEntries_Long(outputNonZeros);
...
*/
	...
	// output ArrayList<NonZeroEntry_Long> nonZeros and int[] array of zero indices
	public static void createSynData_SparseForm(ArrayList<NonZeroEntry_Long> nonZeros, ArrayList<ZeroIndexInterval> zeroIndices, int numNonZeros,  long inputSize, double mean, double stdDev, int seed) {
		Random rand = new Random(seed);
	...
	public static void addGeometricNoise(ArrayList<NonZeroEntry_Long> inputNonZeros, long startIndex, double[] data, double[] output, double alpha, int seed) {
		copyToArray(inputNonZeros, data, startIndex);
		addGeometricNoise(data, output, alpha, seed);	
	}
	
	private static void copyToArray(ArrayList<NonZeroEntry_Long> inputNonZeros, double[] data, long startIndex) {
		long currPosition=0;
		double currVal = 0;
		int currArrayIndex = 0;
		for (int k=0; k<inputNonZeros.size(); k++) { 
			NonZeroEntry_Long entry = inputNonZeros.get(k);
			if (entry.getPosition() >= startIndex) {
				currPosition =  entry.getPosition();
...
//		int i = startIndex;
//		for (; i<currPosition; i++) data[i]=0;
//		
		for (int i=0; i<data.length; i++) {
			if (i+startIndex == currPosition) {
				data[i] = currVal;
				currArrayIndex++;
				currPosition =  inputNonZeros.get(currArrayIndex).getPosition();
				currVal = (int) inputNonZeros.get(currArrayIndex).getValue();
			}
		}
	}
	...
	public static int anonymize(ArrayList<NonZeroEntry_Long> nonZeros, long inputSize, ArrayList<NonZeroEntry_Long> output, Mechanism mechanism, int theta, int K, double epsilon, int noiseSeed, int sampleSeed, int zeroPosSeed)  {	
		double alpha = Math.exp(-epsilon/sensitivity);
		
		int sampleSize;
		Sampling sampling=null;
		if (mechanism == Mechanism.Filter2) {	
		...
		sampleSize = sampling.anonymize(nonZeros, inputSize, output);
		return sampleSize;
	}
...
//	public static DyadicRange_Sparse_1d anonymizeDyadicRanges(ArrayList<NonZeroEntry_Long> nonZeros, int inputSize, Mechanism mechanism, int theta, int sampleSize, double epsilon, int noiseSeed, int sampleSeed, int zeroPosSeed)  {
...
//				pw.println(trueSubsetSum + ", " + estSubsetSum);	
	...
	public static void computeRangeQueryError(ArrayList<NonZeroEntry_Long> input, ArrayList<NonZeroEntry_Long> output, long startRangeIndex, int rangeSize, ErrorProfile ep) {
		int startInputIndex = 0;
		for (; startInputIndex < input.size(); startInputIndex++) {
			if (input.get(startInputIndex).getPosition() >=  startRangeIndex) {
				break;
			}
		}
		
		int endInputIndex = startInputIndex;
		for (; endInputIndex < input.size(); endInputIndex++) {
			if (input.get(endInputIndex).getPosition() >= rangeSize + startRangeIndex) {
				break;
			}
		}
		endInputIndex--;
		...
		for (; startOutputIndex < output.size(); startOutputIndex++) {
			if (output.get(startOutputIndex).getPosition() >= startRangeIndex) break;
		}
		
		int endOutputIndex = startOutputIndex;
		for (; endOutputIndex < output.size(); endOutputIndex++) {
			if (output.get(endOutputIndex).getPosition() >= rangeSize + startRangeIndex) break;
		}
		endOutputIndex--;
	...
	public static void computeRangeQueryError(ArrayList<NonZeroEntry_Long> input, double[] output, int rangeSize, ErrorProfile ep) {
		int startInputIndex = 0;
		int endInputIndex = 0;
		for (; endInputIndex < input.size(); endInputIndex++) {
	...
	public static long readFreqMatrixData(String fileName, ArrayList<NonZeroEntry_Long> data) {
		long numEntries = 0;
		try {
			BufferedReader reader=null;
			reader = new BufferedReader(new FileReader(fileName));
			numEntries = Long.valueOf(reader.readLine());
			...
			while (true) {
				...
				if (line == null)
					...
					if (tokens.length == 2) {
						position = Long.valueOf(tokens[0]);
...
//		int numUpdates = 0;
		...
		public ErrorProfile(int numQueries, boolean useMedian) {
			...
			useMedianRelativeErrors = useMedian;
		}
