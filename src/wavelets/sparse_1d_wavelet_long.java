...
public class sparse_1d_wavelet_long {
	int logm;
	...
	public sparse_1d_wavelet_long(int logm, boolean runInverse) {
		this.logm = logm;
		...
		for (int i=0; i<=logm; i++) {
			transformCoeff[i] = new Pair(new Coeff1_Long(0,0), 0);
		}
	}
	
	public sparse_1d_wavelet_long(int logm, HashMap<Integer,Double> map, boolean runInverse) {
		this.logm = logm;
		this.runInverse = runInverse;
	...
	public sparse_1d_wavelet_long(int logm, double[] coeffArr) {
		this.logm = logm;
		runInverse = true;
		normalize = new double[logm+1];
		for (int i=0; i<logm; i++) {
	...
	public Pair[] haar(long pos) {
		Pair[] result = transformCoeff;
		for (int i=0; i<logm; i++) {
			Coeff1_Long coeff = (Coeff1_Long) result[i].coeff;
			coeff.level = i;
			coeff.offset = (pos >> (i+1));
			result[i].setValue(1.0- (((pos>>i)&1)<<1));
		}
		Coeff1_Long coeff = (Coeff1_Long) result[logm].coeff; 
		coeff.level = logm;
		coeff.offset = 0;
	...
	public void inverse(double[] data, long startIndex) {
		//int m = (1 << logm);
		...
		for (long i=startIndex; i<startIndex+data.length; i++) {
			...
			for (int j=0; j<hx.length; j++) {
				long offset = ((Coeff1_Long) hx[j].coeff).offset;
				int level = ((Coeff1_Long) hx[j].coeff).level;
				long coeff;
				if (level==logm) coeff = 0;
				else coeff = (1 << (logm-1-level)) + offset;
// cast int for running time only, accuracy is wrong				
				sum += (hx[j].value*coeffs[(int) coeff]);
			}
			if (data != null)
				data[(int) (i-startIndex)] = sum;
		}
	}
	...
	public void inverse_Time() {
	 ...
	 public void update(long pos, double val) {
		Pair[] tran = transform(pos, val);
		for (Pair p: tran) {
			Coeff1_Long coeff = (Coeff1_Long) p.coeff;
			...
			if (runInverse)
				coeffs[(int) coeffInt] += value;
			else {
				if (coeffMap.containsKey(coeffInt))
					coeffMap.put((int) coeffInt, coeffMap.get(coeffInt)+value);
				else
					coeffMap.put((int) coeffInt, value);
			}
		}
	 }
	
	private Pair[] transform(long pos, double val) {
		Pair[] hx = haar(pos);
		Pair[] result = transformCoeff;
...
//	}
	...
	public static class Coeff1_Long extends Coeff {
		int level;
		long offset;
		public Coeff1_Long(int xlevel, long offset) {
