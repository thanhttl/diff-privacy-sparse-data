 ...
 *
 */
public class Filtering_Signed extends Sampling {
	double theta;
	
	public Filtering_Signed(double alpha, double theta, int noiseSeed, int sampleSeed, int zeroPosSeed) {
		this.alpha = alpha;
		this.theta = theta;
...
//	}
	...
	public double sampleZeroEntry(double prob) {
		double sample = Math.ceil(Math.log(1-prob)/Math.log(alpha) + theta - 1);
		return sample;
	}
	
	public int getNumZerosSelected(int numZeros) {
		double thetaProb = Math.pow(alpha, theta)/(1+alpha); //p
...
//			numUpgrades = (int) Math.round(noiseRand.nextGaussian()*Math.sqrt(variance) + thetaProb*numZeros);
		if (numUpgrades < 0)
			numUpgrades = 0;
		else if (numUpgrades > numZeros)
			numUpgrades = numZeros;
		return numUpgrades;
	}
	...
	public static void main(String[] args) {
		...
		for (double theta: thetas ) {
			...
			for (int j=0; j<numRepeats; j++) {
				Filtering_Signed sampling = new Filtering_Signed(alpha, theta, seed, 98654, 3242);
