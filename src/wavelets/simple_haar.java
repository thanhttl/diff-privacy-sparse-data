...
<p>
  ...
  // adding noise results in inconsistent wavelet coefficients.
  // what can be inferred from this.
  public void addLaplacian(double scale, boolean isWeighted, Random rand) {
	  int depth = (int) (Math.log(data.length)/Math.log(2));
	  
	  if (!isWeighted) {
		  double newScale = scale*depth;
		  haar_value += Laplace.getSample(newScale, rand); //Math.round(Laplace.getSample(scale, rand));
		  for (int i=0; i<coef.size(); i++) {
			  double[] oldVal = (double[]) coef.get(i);
			  for (int j=0; j<oldVal.length; j++) {
				  oldVal[j] += Laplace.getSample(newScale, rand); //Math.round(Laplace.getSample(scale, rand));
			  }
		  }	
	  } 
	  else { // weighted as in differential privacy - wavelet paper
		  int currWeight =  1 << (depth);
		  double newScale = scale*(1+depth); // this is \lambda in the paper
		  haar_value += Laplace.getSample(newScale/currWeight, rand); //Math.round(Laplace.getSample(scale/currWeight, rand));
		  for (int i=0; i<coef.size(); i++) {
			  double[] oldVal = (double[]) coef.get(i);
			  for (int j=0; j<oldVal.length; j++) {
				  oldVal[j] += Laplace.getSample(newScale/currWeight, rand); //Math.round(Laplace.getSample(scale/currWeight, rand));
			  }	  
			  currWeight /= 2;
		  }	
	  }
  ...
  public static void main(String[] args) {
	  double[] input = {9, 3, 6, 2, 8, 4, 5, 7};
	  simple_haar wavelet = new simple_haar();
	  wavelet.wavelet_calc(input);
	  wavelet.pr();
	  wavelet.addLaplacian(2, true, new Random(12345));
