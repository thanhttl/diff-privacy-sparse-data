...
public class sparse_2d_wavelet {
	int logx;
	int logy;
...
//	double[] power;
	...
	public sparse_2d_wavelet(int logx, int logy) {
		this.logx = logx;
		this.logy = logy;
...
//			power[i] = Math.pow(2, i);
		...
		coeffs = new HashMap<Integer, Double>();
	...
	public void update(int x, int y, double z) {
		...
		for (Pair p: tran) {
			...
			double val = p.value;
			if (!coeffs.containsKey(coeffXY))
				coeffs.put(coeffXY, val);
			else
				coeffs.put(coeffXY, coeffs.get(coeffXY)+val);				
		}
	}
	...
	public double inverse(int x, int y) {
		...
		for (Pair p: coeffTran) {
			...
			if (coeffs.containsKey(coeffXY))
				result += p.value*coeffs.get(coeffXY)/normalize[xl+yl];
		}
		return result;
	}
			...
			for (int j=0; j<=logx; j++) {
				...
				for (int u=0; u<numXEntries; u++) {
					for (int v=0; v<numYEntries; v++) {
						...
						if (coeffs.containsKey(coeffXY)) {
							coeffs.put(coeffXY, coeffs.get(coeffXY)+Laplace.getSample(scale/weight, rand));
						}
						else {
							coeffs.put(coeffXY, Laplace.getSample(scale/weight, rand));
						}
					}
				}
